...
describe('Request specs', () => {
  ...
  it('sends a get request with correct method, url and headers', () => {
    ...
    req.requestHeaders['X-LOCFLOW'].should.eq('true')
  })
  ...
  it('triggers success even if timeout was triggered', (done) => {
    setRequestTimeout(5)
    let req = Request.get('locflow.io')
    let onTimeout = sinon.spy()
    let onSuccess = sinon.spy()
    req.on('timeout', onTimeout)
    req.on('success', onSuccess)
    setTimeout(() => {
      expect(onTimeout.called).to.be.true
      requests[0].respond(200, {}, '<h1>Hello</h1>')
  ...
  it('aborts the request with the `abort` method', () => {
    let req = Request.get('/home')
    req.abort()
    expect(req.aborted).to.be.true
  })

  it('doesnt trigger timeout if the request was aborted', (done) => {
    setRequestTimeout(5)
    let onTimeout = sinon.spy()
    let req = Request.get('/home')
    req.on('timeout', onTimeout)
  ...
  /*
  ** Sinon's request mock implementation doesn't allow us to send a response to 
  ** an aborted request, which makes sense. Because of this, we can't test 
  ** that the events (success, error, redirect) are not being called once the 
  ** request is aborted.
  */
})
